    ...
    if (connection === "close") {
        ...
        switch (reason) {
            case DisconnectReason.badSession:
                console.log(`File Sesi Bermasalah, Silakan Hapus ${session} dan Scan Lagi`);
                await sock.logout();
                break;
            ...
            case DisconnectReason.connectionLost:
                console.log("Koneksi Hilang dari Server, menghubungkan kembali dalam beberapa detik...");
                await new Promise((resolve) => setTimeout(resolve, 5000));
                await connectToWhatsApp();
                break;

            case DisconnectReason.connectionReplaced:
                console.log("Koneksi Digantikan, Sesi Baru Dibuka. Silakan Tutup Sesi Lama.");
                await sock.logout();
                break;

            case DisconnectReason.loggedOut:
                console.log(`Perangkat Keluar, Silakan Hapus ${session} dan Scan Lagi.`);
                try {
                    if (sock) {
                        await sock.logout(); // Logout sebelum hapus folder
                ...
                // Hapus isi folder tanpa menghapus foldernya
                if (fs.existsSync(session)) {
                    fs.readdirSync(session).forEach(file => {
                        const curPath = path.join(session, file);
                    ...
                    console.log(`Isi folder ${session} telah dihapus.`);
                }

                await connectToWhatsApp();
                break; // ✅ Tambahkan break di sini agar tidak lanjut ke case lain
            ...
            default:
                console.log(`Alasan Disconnect Tidak Diketahui: ${reason} | ${lastDisconnect?.error}`);
                await connectToWhatsApp();
                break;
        }
